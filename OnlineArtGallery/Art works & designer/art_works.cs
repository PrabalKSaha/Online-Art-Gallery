...
namespace art2
{
    public partial class art_works : UserControl
    {
        ...
        {
            InitializeComponent();
            connection.ConnectionString = @"Provider=Microsoft.ACE.OLEDB.12.0;Data Source=E:\Works\visc#\art2\LIB.accdb;Persist Security Info=False;";
   ...
   ///
            ...
            try
            {
                connection.Open();
                string[] tasks;
                string sql = "SELECT Price FROM Painting";
                using (OleDbCommand cmd = new OleDbCommand(sql, connection))
                {
                    using (OleDbDataReader dataReader = cmd.ExecuteReader())
                    {
        ...
        void loadpicture()
        {
            ...
            make_null();
            string path = @"C:\Users\sid\Pictures\";
